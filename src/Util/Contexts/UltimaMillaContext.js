...
function convertData(trucks, guias, dateFilter) {
    var array = []
    array.push(trucks.map(t => Depot(t.m_nIdUnidad,-115.45446,32.62781 , dateFilter.start, dateFilter.finish)));
...
async function obtenerRutas(truck, paquetes) {
    var guias = []
    ...
    console.log(xtour.planTours({
        "locations": convertData(truck, guias),
        "orders":
            guias.map((p, index) => (
                {
                    "$type": "VisitOrder",
                ...
                truck.map(t => (
                    {
                        ...
                        "startLocationId": t.m_nIdUnidad,
                        "endLocationId": t.m_nIdUnidad
                    }
                ))
        },
        "distanceMode": {
