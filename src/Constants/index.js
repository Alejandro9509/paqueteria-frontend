...
export const TICKET_ZEBRA_TEMPLATE = (guia, paquete, indexPartida,paquetesTotales, indexQR) => {
    ...
    const fechaGuia = new Date(guia?.m_dFecha+"T00:00:00");
    fechaGuia.getUTCMonth()
    const opciones = { year: 'numeric', month: 'long', day: 'numeric' };//timeZone: "America/Los_Angeles"
    const fechaLocalizada = fechaGuia.toLocaleDateString('es-MX', opciones);
    if(rfc === 'PLG090716IA7') {
        return (`CT~~CD,~CC^~CT~
^XA~TA000~JSN^LT0^MNW^MTT^PON^PMN^LH0,0^JMA^PR4,4~SD15^JUS^LRN^CI0^XZ
^XA
...
${origen.length > 17 ? (
            `
    ^FT550,750^A0R,35,45^FH\\^FD${origen.origen(0, 17)}^FS
    ^FT510,750^A0R,35,45^FH\\^FD${origen.origen(17, 34)}^FS
    `
        ) : `^FT550,750^A0R,35,45^FH\\^FD${origen}^FS`}

${destino.length > 17 ? (
            `
    ^FT450,750^A0R,35,45^FH\\^FD${destino.substring(0, 17)}^FS
    ^FT410,750^A0R,35,45^FH\\^FD${destino.substring(17, 34)}^FS
    `
        ) : `^FT450,750^A0R,35,45^FH\\^FD${destino}^FS`}
